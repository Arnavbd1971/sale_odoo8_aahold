...
class sale_order(osv.osv):
    ...
    def _amount_all(self, cr, uid, ids, field_name, arg, context=None):  
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            res[order.id]['quantity_total'] = cur_obj.round(cr, uid, cur, val2)
    ...
    _columns = {
        ...
        'amount_total': fields.function(_amount_all_wrapper, digits_compute=dp.get_precision('Account'), string='Total Price',
            store={
                'sale.order': (lambda self, cr, uid, ids, c={}: ids, ['order_line'], 10),
                'sale.order.line': (_get_order, ['price_unit', 'tax_id', 'discount', 'product_uom_qty'], 10),
            },
        ...
        'quantity_total': fields.function(_amount_all_wrapper, string='Total Quantity', type='integer',
    ...
    def onchange_signature(self, cr, uid, ids, signature, context=None):
        ...
        if signature_id : 
            service_obj= self.pool.get('signature_upload.model').browse(cr, uid,signature_id,context=context)
    ...
    def manual_invoice(self, cr, uid, ids, context=None):
        ...
        # create invoices through the Proforma Invoices' workflow
...
class sale_order_line(osv.osv):
    ...
    _columns = {
        ...
        'product_uom_qty': fields.integer('Quantity', digits_compute= dp.get_precision('Product UoS'), required=True, readonly=True, states={'draft': [('readonly', False)]}),
    ...
    def create(self, cr, uid, values, context=None):
        if values.get('order_id') and values.get('product_id') and  any(f not in values for f in ['name', 'price_unit', 'product_uom_qty', 'product_uom']):
            ...
            defaults = self.product_id_change(cr, uid, [], order['pricelist_id'][0], values['product_id'],
                qty=int(values.get('product_uom_qty', False)),
